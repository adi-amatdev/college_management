...
class DepartmentList(APIView):
    def get(self,request):
        model = Departmentlist.objects.all()
        serializer = DepartmentListSerializer(model,many=True)
        return Response(serializer.data)
...
class DepartmentCourseInfo(APIView):
    def get(self,request):
        model = Departmentcourseinfo.objects.all()
        serializer = DepartmentCourseInfoSerializer(model,many=True)
        return Response(serializer.data)
    
    def post(self,request):
        serializer = DepartmentCourseInfoSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data,status=status.HTTP_201_CREATED)
        return Response(serializer.errors,status=status.HTTP_404_NOT_FOUND)
...
class TeacherList(APIView):
    def get(self,request):
        model = Teacherslist.objects.all()
